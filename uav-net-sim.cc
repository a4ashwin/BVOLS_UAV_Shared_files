...
long g_cmd_num = 0;
float g_cmd_avg_delay_curr= 0;
int avg_changed_lte = 0;
int avg_changed_wifi = 0;
...
void MonitorLTE(uint16_t cellId, uint16_t rnti, double rsrp, double sinr, uint8_t componentCarrierId)
{
	if(avg_changed_lte == 1)
	{
	avg_changed_lte =0;
	FILE * pFile;
	pFile = fopen ("/home/ubuntu/FlyNetSim/FlySim/flight_logs.txt","a");
	fprintf(pFile, "\n");
	fprintf(pFile, "LteSignalStrength= %f dBm*** LteSINR= %f dBm \n", rsrp, sinr); 
	fprintf(pFile, "\n");
	fclose (pFile);
	//std::cout<< "Average delay: " << g_cmd_avg_delay_curr << std::endl;
	//std::cout<< "Channel Frequency: "<< channelFreqMhz << "\t Tx: "<< txVector.GetMode()<< "\t Signal=" << signalNoise.signal<< "\t Noise=" << signalNoise.noise<< std::endl;
	}
}
void MonitorWifi(Ptr<const Packet> packet, uint16_t channelFreqMhz, WifiTxVector txVector, MpduInfo aMpdu, SignalNoiseDbm signalNoise)
{
	//std::cout << "IN MONITOR +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++";
	//std::cout<< "Channel Frequency: "<< channelFreqMhz << std::endl;
	
	if(avg_changed_wifi == 1)
	{
	avg_changed_wifi = 0;
	FILE * pFile;
	pFile = fopen ("/home/ubuntu/FlyNetSim/FlySim/flight_logs.txt","a");
	fprintf(pFile, "\n");
	fprintf(pFile, "WifiSignalStrength= %f dBm*** WifiNoise= %f dBm \n", signalNoise.signal, signalNoise.noise); 
	fclose (pFile);
	std::cout<< "Average delay: " << g_cmd_avg_delay_curr << std::endl;
	std::cout<< "Channel Frequency: "<< channelFreqMhz << "\t Tx: "<< txVector.GetMode()<< "\t Signal=" << signalNoise.signal<< "\t Noise=" << signalNoise.noise<< std::endl;
	}
...
{	
	...
	//char* homePath = const_cast<char*>(HOME.c_str());
	FILE * pFile;
	pFile = fopen ("/home/ubuntu/FlyNetSim/FlySim/flight_logs.txt","a");
	...
	strcpy(s, string);
	  ...
	  //logs added to file home/ubuntu/FlyNetSim/FlySim/flight_logs.txt
	  fprintf(pFile, "\n"); 
	  fprintf(pFile, "============GCS to UAV: Control Commands===========\n"); 
	  fprintf(pFile, " Publish MSG : %s \n", string); 
	  ...
	  if((g_cmd_end - g_cmd_start) > 1000)   // print average network packet delay for command packets, in every sec
	  {
		...
		g_cmd_avg_delay_curr = g_cmd_avg_delay;
              printf(">>>>>> Average Network Delay for command packets: %f MilliSec.\n", g_cmd_avg_delay);
		//logs added to file home/ubuntu/FlyNetSim/FlySim/flight_logs.txt
		fprintf(pFile, "\n"); 
	      fprintf(pFile, ">>>>>> Average Network Delay for command packets: %f MilliSec.\n", g_cmd_avg_delay); 
	      avg_changed_wifi = 1;
		avg_changed_lte = 1;
	      //reset variables
	      g_cmd_tot_delay = 0;
		...
		//logs added to file home/ubuntu/FlyNetSim/FlySim/flight_logs.txt
	  fprintf(pFile, "\n");
	  fprintf(pFile, "============= UAV to GCS: Telemetry Packets==============\n"); 	
	  fprintf(pFile, " Publish MSG : %s \n", string); 
	  fprintf(pFile, "\n");
        ...
        else if(string[3] == 'S')   // for SENSOR  // This section may need revision
        {
          ...
          while(true)
          {
   		...
   		//logs added to file home/ubuntu/FlyNetSim/FlySim/flight_logs.txt
	    fprintf(pFile, "ZMQ SENSOR DATA SIZE: %d \n", size); 
	    fprintf(pFile, "\n"); 
		...
		//logs added to file home/ubuntu/FlyNetSim/FlySim/flight_logs.txt
	    fprintf(pFile, "POPPED MSG from ZMQ: %s \n", strVZ); 
	      fprintf(pFile, "\n"); 
		...
		//logs added to file home/ubuntu/FlyNetSim/FlySim/flight_logs.txt
	    fprintf(pFile, " Publish MSG from ZMQ: %s \n", strVZ); 
	    fprintf(pFile, "\n"); 
...
void RcvPktTrace(){
	...
	Config::ConnectWithoutContext("/NodeList/*/DeviceList/*/$ns3::WifiNetDevice/Phy/MonitorSnifferRx", MakeCallback(&MonitorWifi));
	Config::ConnectWithoutContext("/NodeList/*/DeviceList/*/ComponentCarrierMapUe/*/LteUePhy/ReportCurrentCellRsrpSinr", MakeCallback(&MonitorLTE));
...
void* rcvTelemetry(void *arg)
{
  ...
  while(1)
  {
	...
	//logs added to file home/ubuntu/FlyNetSim/FlySim/flight_logs.txt
    //fprintf(pFile, " TELEMETRY Recvd: Size %ld \n", strlen(string)); 
    //fprintf(pFile, "\n"); 
    printf("============TELEMETRY Recvd at GCS============\n");
    printf("%s \n", string);
	//logs added to file home/ubuntu/FlyNetSim/FlySim/flight_logs.txt
    fprintf(pFile, " TELEMETRY Recvd: %s \n", string); 
      fprintf(pFile, "\n"); 
	...
	//logs added to file home/ubuntu/FlyNetSim/FlySim/flight_logs.txt
	fprintf(pFile, ">>>>>> X-Value:  %s  ;  Y-Value:  %s <<<<<<\n", curX, curY); 
    	 fprintf(pFile, "\n"); 
...
int main (int argc, char *argv[])
{
  ...
  //mobilityLte.SetMobilityModel ("ns3::BuildingsMobilityModel");
